...
public interface ActParticipantRecordRepository extends JpaRepository<Act_Participant_Record, Integer> {
	...
	@Query("select apr from Act_Participant_Record apr where apr.act_id = ?1 and apr.is_canceled = 0")
	public List<Act_Participant_Record> getAct_ParticipantRecordsCountByactId(int act_id);
	
	@Query(value="select new com.vmoving.dto.ParticipantInfo(apr.user_id,apr.act_id,u.openid,u.nickName,u.avatarUrl,apr.joindate,apr.user_status_id,apr.comments) from Act_Participant_Record apr left join UserBasicData u on apr.user_id = u.user_id"
	...
	/**
	 * Gets all joined activities            
	 * @param user_id
	 * @return A user has joined activities
	 */
